...
## Corpus Check
- 119 files · ~86,267 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1265 nodes · 1784 edges · 79 communities (73 shown, 6 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 48 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `62c8f2ae`
...
- TestBocLinkIntegration

## God Nodes (most connected - your core abstractions)
1. `BaseSpider` - 45 edges
2. `CamoufoxEngine` - 26 edges
3. `WorkersClient` - 24 edges
4. `Config` - 18 edges
5. `chamDiem()` - 18 edges
6. `CrawlOrchestrator` - 17 edges
7. `napLead()` - 17 edges
8. `ScraplingEngine` - 15 edges
9. `is_safe_url()` - 14 edges
10. `is_lead_signal()` - 13 edges
...
### Community 3 - "is_lead_signal"
Cohesion: 0.06
Nodes (23): BlackHatWorldSpider, BrandsVietnamSpider, is_lead_signal(), Forum Spider — Crawls marketing forums for lead signals.…, Spider for WarriorForum — one of the largest internet marketing forums., Spider for VOZ forum marketing section (voz.vn). Vietnamese tech/marketing…, Spider for BrandsVietnam.com — Premier Vietnamese marketing & branding…, Check if text contains signals of a job opportunity. (+15 more)

### Community 4 - "TestLeadSignalKeywords"
Cohesion: 0.25
Nodes (5): Test keyword list completeness., VN keyword list should not be empty., EN keyword list should not be empty., Critical VN keywords must be present., TestLeadSignalKeywords

### Community 5 - "spiders/__init__.py"
Cohesion: 0.05
Nodes (28): CareerVietSpider, ContraVNSpider, FastlanceSpider, FreelancerVNSpider, GigHitSpider, JobBoardVNSpider, JobsGoSpider, PeoplePerHourSpider (+20 more)
...
Cohesion: 0.11
Nodes (39): locTrungDaCo(), locTrungTrongLo(), napNhieuLead(), chuanBiMessageQuetNguon(), xuLyLayBai(), xuLyMotMessage(), xuLyQuetNguon(), capNghenguon() (+31 more)
...
Cohesion: 0.09
Nodes (17): Return all enabled sources., Filter sources by engine type., CrawlOrchestrator, main(), Gom lỗi theo kind để biết nên xử lý gì tiếp (vd: nhiều 'blocked' = dừng hammer)., Cập nhật circuit breaker + baseline sau mỗi run. Circuit: quan tâm "có fetch…, Run all enabled spiders. engines: filter by engine type ["scrapling_fast",…, Run only social media spiders (TikTok + Facebook). These need Camoufox and are… (+9 more)

### Community 21 - "BaseSpider"
Cohesion: 0.07
Nodes (25): ABC, CamoufoxEngine, Camoufox Engine — Anti-detect browser for social media crawling.…, Anti-detect browser engine using Camoufox. Designed for platforms with heavy…, Engines package — Scrapling + Camoufox hybrid., Scrapling Engine — Fast + Stealth crawling via Scrapling framework.…, Fetch multiple URLs with concurrency limit and delay between batches., Dual-mode Scrapling engine: - fast mode: FetcherSession with browser… (+17 more)
...
### Community 28 - "is_safe_url"
Cohesion: 0.08
Nodes (16): Unit Tests — URL guard (SSRF allowlist + robots.txt)…, 169.254.169.254 = cloud metadata endpoint — SSRF kinh điển., TestIsSafeUrl, TestParseRobots, TestPathAllowed, is_safe_url(), parse_robots(), path_allowed() (+8 more)
...
Cohesion: 0.09
Nodes (17): AsyncClient, Config, Crawl Agent Configuration ========================= Central configuration for…, Agent configuration — all settings from env or defaults., Crawl Agent Orchestrator — Coordinates all spiders and engines.…, fetch_feedback(), main(), Gọi Workers API, trả feedback dạng {positive, negative, updated_at}. (+9 more)
...
Cohesion: 0.10
Nodes (23): classify_batch(), classify_jobs_fast(), classify_rule_based(), _llm_confidence(), main(), Fallback: Rule-based classification., Classify all jobs using batch processing., Confidence trung thực: derive từ score, không hardcode. Score càng cực đoan… (+15 more)

### Community 41 - ".fetch"
Cohesion: 0.25
Nodes (4): Fetch and extract specific data using CSS selectors. selectors: {"title":…, Fast fetch — impersonate Chrome/Firefox TLS fingerprint. Good for: forums, job…, Stealth fetch — full browser with Cloudflare bypass. Good for: vLance, sites…, Smart fetch — try fast first, escalate to stealth if needed. engine: "fast" |…
...
### Community 46 - "classify"
Cohesion: 0.18
Nodes (8): Unit Tests — Error envelope (recovery contract)…, TestClassify, TestMakeError, classify(), make_error(), Structured error envelope — lỗi phải mang theo cách xử lý.…, Phân loại một lỗi fetch. HÀM THUẦN. @returns {"kind": str, "retryable": bool,…, Tạo error envelope chuẩn cho stats["errors"]. HÀM THUẦN. @param phase "listing"…
...
### Community 51 - ".crawl"
Cohesion: 0.17
Nodes (6): Generate dedup key from URL., Check if URL has been seen before in this run., Format extracted data into lead payload for Workers API. Matches the format…, Full crawl cycle: 1. Fetch listing pages 2. Extract individual post links 3.…, Parse a listing/search page → extract individual job/post links. Returns:…, Parse a detail page → extract full job/post content. Returns: {"noiDung":…
...
### Community 56 - "TestLeadFormatContract"
Cohesion: 0.33
Nodes (4): Test lead format matches Workers API contract., Lead must have required fields for /api/demand/nap., noiDung must not be empty (nap-lead.js requires >= 20 chars)., TestLeadFormatContract
...
### Community 78 - "TestBocLinkIntegration"
Cohesion: 0.50
Nodes (3): Test integration with existing boc-link.js logic (conceptual)., URL dedup should normalize URLs., TestBocLinkIntegration
...
- **Why does `Config` connect `orchestrator.py` to `is_lead_signal`, `TestLeadSignalKeywords`, `TestBocLinkIntegration`, `CrawlOrchestrator`, `BaseSpider`, `TestLeadFormatContract`?**
  _High betweenness centrality (0.045) - this node is a cross-community bridge._
- **Why does `BaseSpider` connect `BaseSpider` to `is_lead_signal`, `orchestrator.py`, `spiders/__init__.py`, `.crawl`, `VLanceSpider`, `FreelancerComVNSpider`, `Job123Spider`, `is_safe_url`?**
  _High betweenness centrality (0.034) - this node is a cross-community bridge._
- **Why does `WorkersClient` connect `orchestrator.py` to `CrawlOrchestrator`, `BaseSpider`?**
  _High betweenness centrality (0.028) - this node is a cross-community bridge._
- **Are the 28 inferred relationships involving `BaseSpider` (e.g. with `Config` and `CamoufoxEngine`) actually correct?**
  _`BaseSpider` has 28 INFERRED edges - model-reasoned connections that need verification._
