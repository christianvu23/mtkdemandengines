...
Cohesion: 0.06
Nodes (59): DINH_DANG, moTaLead(), server, transport, boDau(), chuanHoaKey(), chuanHoaText(), doTuongDong() (+51 more)
...
### Community 3 - "BaseSpider"
Cohesion: 0.06
Nodes (22): ABC, Camoufox Engine — Anti-detect browser for social media crawling.…, BaseSpider, Base Spider — Abstract base for all crawl spiders.…, Full crawl cycle: 1. Fetch listing pages 2. Extract individual post links 3.…, Abstract base spider that all source-specific spiders inherit from. Handles:…, Parse a listing/search page → extract individual job/post links. Returns:…, Parse a detail page → extract full job/post content. Returns: {"noiDung":… (+14 more)
...
Cohesion: 0.06
Nodes (22): BehanceVNSpider, CareerVietSpider, ContraVNSpider, FastlanceSpider, FreelancerVNSpider, JobBoardVNSpider, Freelancer Spider — Crawls Vietnamese & international freelancer job boards.…, Spider for Freelancer.vn — another Vietnamese freelance marketplace. (+14 more)
...
Cohesion: 0.06
Nodes (30): dotenv, @modelcontextprotocol/sdk, allowScripts, esbuild@0.28.1, workerd@1.20260811.1, dependencies, dotenv, @modelcontextprotocol/sdk (+22 more)
...
Cohesion: 0.19
Nodes (11): Return all enabled sources., CrawlOrchestrator, main(), Run all enabled spiders. engines: filter by engine type ["scrapling_fast",…, Run only social media spiders (TikTok + Facebook). These need Camoufox and are…, Check all systems are operational., Save crawl results to JSON file., CLI entry point for the crawl agent. (+3 more)

### Community 21 - "CamoufoxEngine"
Cohesion: 0.12
Nodes (12): CamoufoxEngine, Anti-detect browser engine using Camoufox. Designed for platforms with heavy…, FacebookGroupSpider, Crawl TikTok by searching multiple queries. Each query gets a separate search…, Spider for TikTok — searches for marketing-related content. Uses Camoufox with…, Spider for Facebook Groups — finds marketing job posts. Uses Camoufox with…, Parse FB Group posts — already JSON from JS extraction., Parse FB post detail. (+4 more)
...
Cohesion: 0.13
Nodes (7): Crawl Agent Orchestrator — Coordinates all spiders and engines.…, Spider for WarriorForum — one of the largest internet marketing forums., WarriorForumSpider, FreelancerComVNSpider, Job123Spider, Spider for Freelancer.com Vietnam — Global platform with VN freelancer section.…, Spider for 123Job.vn — Has Facebook Marketing & Google Ads Freelancer…
...
### Community 28 - "WorkersClient"
Cohesion: 0.19
Nodes (7): Config, Crawl Agent Configuration ========================= Central configuration for…, Agent configuration — all settings from env or defaults., Filter sources by engine type., Workers API Client — Send crawled leads to MTK Demand Engines.…, HTTP client for the MTK Demand Engines Workers API., WorkersClient
...
### Community 36 - "ScraplingEngine"
Cohesion: 0.22
Nodes (5): Engines package — Scrapling + Camoufox hybrid., Scrapling Engine — Fast + Stealth crawling via Scrapling framework.…, Fetch multiple URLs with concurrency limit and delay between batches., Dual-mode Scrapling engine: - fast mode: FetcherSession with browser…, ScraplingEngine
...
- **Why does `dotenv` connect `package.json` to `WorkersClient`?**
